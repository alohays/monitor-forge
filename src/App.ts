...
export class App {
  ...
  async initialize(): Promise<void> {
    ...
    // Apply theme before layout rendering to prevent flash of unstyled content
  ...
  private async applyTheme(): Promise<void> {
    try {
      const { resolvedTheme } = await import('./generated/theme-resolved.js');
      const theme = resolvedTheme as { mode: 'dark' | 'light' | 'auto'; dark: typeof resolvedTheme.dark; light: typeof resolvedTheme.light; panelPosition: string; panelWidth: number; compactMode: boolean };
      const root = document.documentElement;

      // Set data attributes for CSS selectors
      root.setAttribute('data-theme', theme.mode);
      root.setAttribute('data-compact', String(theme.compactMode));
      root.setAttribute('data-panel-position', theme.panelPosition);

      // Determine effective mode for initial paint
